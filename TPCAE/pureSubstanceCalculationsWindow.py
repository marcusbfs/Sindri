    ...
    def calculatePureSubstance(self):
        ...
        if len(self.sname.strip()) > 1 and len(self.eosname.strip()) > 1:
            ...
            if self.compound["Mol. Wt."] is not None:
                self.rholiq = (
                    self.compound["Mol. Wt."] * 1e-3 / self.Vliq
                )  # all densisties are in kg/m3
                ...
                self.plainTextEdit_results.appendPlainText(
                    "Density [kg/m3]: " + maxrhos + " (liquid), " + minrhos + " (vapor)"
                )
            else:
                # TODO
            ...
            if (
                ...
                try:
                    self.IG_properties = IGprop.return_IdealGasProperties(
                        self.Tref,
                        self.T,
                        self.Pref,
                        self.P,
                    ...
                    if self.IG_properties["msg"] is not None:
                        self.plainTextEdit_results.appendPlainText(
                            "Cp calculation is out of bound ({:s})".format(msg)
                        )

                    self.plainTextEdit_results.appendPlainText(
                        "Cp at T [J mol-1 K-1]: " + Cpstr
                    )
                    self.plainTextEdit_results.appendPlainText(
                        "dH_IG [J mol-1]: " + Hstr
                    )
                    self.plainTextEdit_results.appendPlainText(
                        "dS_IG [J mol-1 K-1]: " + Sstr
                    )
                    self.plainTextEdit_results.appendPlainText(
                        "dG_IG [J mol-1]: " + Gstr
                    )
                ...
                try:
                    ...
                    self.residualDelta_vap = self.c.return_delta_ResProperties(
                        self.Pref,
                        self.Tref,
                        self.Vvapref,
                        self.Zvapref,
                    ...
                    self.residualDelta_liq = self.c.return_delta_ResProperties(
                        self.Pref,
                        self.Tref,
                        self.Vliqref,
                        self.Zliqref,
                    ...
                    Hstr = (
                        utils.f2str(self.dH_liq, _decimals, _lt, _gt)
                        + " (liq.), "
                        + utils.f2str(self.dH_vap, _decimals, _lt, _gt)
                        + " (vap.)"
                    )
                    Sstr = (
                        utils.f2str(self.dS_liq, _decimals, _lt, _gt)
                        + " (liq.), "
                        + utils.f2str(self.dS_vap, _decimals, _lt, _gt)
                        + " (vap.)"
                    )
                    Gstr = (
                        utils.f2str(self.dG_liq, _decimals, _lt, _gt)
                        + " (liq.), "
                        + utils.f2str(self.dG_vap, _decimals, _lt, _gt)
                        + " (vap.)"
                    )
                    Ustr = (
                        utils.f2str(self.dU_liq, _decimals, _lt, _gt)
                        + " (liq.), "
                        + utils.f2str(self.dU_vap, _decimals, _lt, _gt)
                        + " (vap.)"
                    )
                    Astr = (
                        utils.f2str(self.dA_liq, _decimals, _lt, _gt)
                        + " (liq.), "
