...
for i in range(m):
    cursor.execute("SELECT R,Q from subgroups where subgroup_name = '{}' order by number".format(groups[i]))
    res = cursor.fetchone()
    Rk[i], Qk[i] = res
    query = "select number from maingroups where subgroups like '%[' || (select number from subgroups where subgroup_name = '{}')|| ']%'".format(groups[i])
...
for _i in range(n):
    ...
    ln_gamma_C[_i] = 1.0 - J[_i] + np.log(J[_i]) - 5.*q[_i]*(1. - J[_i]/L[_i] + np.log(J[_i]/L[_i]))

    _s = 0.0
    for _k in range(m):
        _s += theta[_k]*beta[_i][_k]/s[_k] - e[_k][_i]*np.log(beta[_i][_k]/s[_k])
    ln_gamma_R[_i] = q[_i]*(1. - _s)
